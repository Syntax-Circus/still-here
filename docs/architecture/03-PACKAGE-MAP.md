...
| Password hashing | `Microsoft.Extensions.Identity.Core` | 10.0.11 | [nuget.org/packages/Microsoft.Extensions.Identity.Core/10.0.11](https://www.nuget.org/packages/Microsoft.Extensions.Identity.Core/10.0.11) | `PasswordHasher<T>`, used standalone (not full ASP.NET Core Identity) via `PasswordHasher<string>` behind `IAdminPasswordHasher`. Needed explicitly because `StillHere.Infrastructure` is a plain classlib without the ASP.NET Core shared framework `StillHere.Web` gets implicitly | Phase 02 |
| End-to-end HTTP testing | `Microsoft.AspNetCore.Mvc.Testing` | 10.0.11 | [nuget.org/packages/Microsoft.AspNetCore.Mvc.Testing/10.0.11](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Testing/10.0.11) | `WebApplicationFactory<Program>` for `StillHere.Web.Tests` — exercises the real HTTP auth surface (redirects, cookies) rather than mocking it | Phase 02 |

All versions are locked in `Directory.Packages.props` as of Phase 02.
